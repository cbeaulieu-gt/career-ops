...
/**
 * Strip Chromium/Skia producer metadata from a generated PDF.
 *
 * PDFs generated by headless Chromium embed "Producer: Skia/PDF m{ver} Google LLC"
 * and "Creator: Chromium" in their info dictionary. Enterprise email security
 * gateways (Proofpoint, Mimecast, MS Defender for O365) heuristically flag
 * these strings as suspicious when the document also contains embedded
 * hyperlinks — a pattern shared with phishing PDFs. Setting both fields to
 * empty strings prevents pdf-lib from writing its own producer string on
 * re-save as well.
 */
async function sanitizePDFMetadata(pdfPath) {
  const { readFile: read, writeFile: write } = await import('fs/promises');
  const existing = await read(pdfPath);
  const pdfDoc = await PDFDocument.load(existing);
  pdfDoc.setProducer('');
  pdfDoc.setCreator('');
  const sanitized = await pdfDoc.save();
  await write(pdfPath, sanitized);
}
...
async function generatePDF() {
  ...
  try {
    ...
    // Strip Chromium/Skia producer metadata to prevent false-positive malware
    // detection at enterprise security gateways (Proofpoint, Defender, etc.)
    await sanitizePDFMetadata(outputPath);
    ...
    console.log(`🔒 Metadata sanitized (Chromium producer/creator stripped)`);
    console.log(`📊 Pages: ${pageCount}`);
    console.log(`📦 Size: ${(finalSize / 1024).toFixed(1)} KB`);

    return { outputPath, pageCount, size: finalSize };
  } finally {
    await browser.close();
  }
}
