...
try {
  ...
  // The exact SDK error depends on network availability: an online run with
  // the fake key reports an authentication error, while a hermetic/offline run
  // reports `fetch failed`. Both prove the evaluator got past file loading and
  // attempted the API call, which is the boundary this regression covers.
  if (stderr.includes('Gemini API error:')) {
