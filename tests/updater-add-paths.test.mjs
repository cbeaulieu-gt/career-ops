...
// ── 6d. preserved files stay out of literal force-add staging ────────────
//     expandToShippedFiles keeps exclusion pathspecs for the later scoped
//     commit. addPaths must interpret those exclusions before switching Git to
//     literal-path mode, or Git treats `:(exclude)...` as a missing filename.
{
  const { dir, g, ctx } = makeRepo();
  mkdirSync(join(dir, 'docs'));
  writeFileSync(join(dir, 'docs/README.md'), 'v1');
  ...
  writeFileSync(join(dir, 'docs/README.md'), 'updated');
  writeFileSync(join(dir, 'docs/KEEP.md'), 'the preserved local edit');
  const expanded = expandToShippedFiles(
    ['docs/', ':(exclude)docs/KEEP.md'],
    'HEAD',
    ctx,
  );
  ...
  const staged = stagedPaths(g);
  if (!threw && staged.has('docs/README.md') && !staged.has('docs/KEEP.md')) {
    pass('literal force-add stages updates while honoring preserved exclusions');
  } else {
    fail(
      `preserved exclusion broke staging: error=${threw?.message.split('\n')[0] || '(none)'} ` +
      `staged=${[...staged].join(', ') || '(nothing)'}`
    );
  }
  rmSync(dir, { recursive: true, force: true });
}
