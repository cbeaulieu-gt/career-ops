...
- **Bundled** — promoted into `plugins/` (shipped, auto-updated). Reserve for
  broadly-useful, well-tested plugins (how `apify`/`gmail`/`h1b-sponsor`/`notion`
  were absorbed). Adds a maintenance commitment + a `config/plugins.example.yml`
  block + an `.env.example` entry.

Adzuna and JSearch in this fork are unlisted direct installs maintained in
standalone repositories. They are neither bundled nor registry-reviewed; users
must supply and consent to an exact commit explicitly.
...
A bundled plugin (`plugins/apify`, `plugins/gmail`, `plugins/h1b-sponsor`, `plugins/notion`) is a
