...
- **Bundled** — promoted into `plugins/` (shipped, auto-updated). Reserve for
  broadly-useful, well-tested plugins (how `adzuna`/`apify`/`gmail`/`jsearch`/`notion`
...
A bundled plugin (`plugins/adzuna`, `plugins/apify`, `plugins/gmail`, `plugins/jsearch`, `plugins/notion`) is a
